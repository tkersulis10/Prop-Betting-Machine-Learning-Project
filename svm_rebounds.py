...
def featurize_instance(player_id, date_string, season_obj, players):
    ...
    reb_totals = extract_stat_from_log('trb', valid_games)
    mp_totals = [0.0 if game=="DNP" else float(game['mp'].split(":")[0]) for game in valid_games]
    orb_totals = extract_stat_from_log('orb', valid_games)
    rebounds = sum(reb_totals)
    rpg = rebounds/len(valid_games)
    orebounds = sum(orb_totals)
    orpg = orebounds/len(valid_games)
    mp = sum(mp_totals)
    mp_average = mp/len(valid_games)

    #Basic cumulative season stats
    season = [rpg, orpg, mp_average]
    feature_vector += season

    reb_pct_totals = extract_stat_from_log('trb_pct', valid_games)
    orb_pct_totals = extract_stat_from_log('orb_pct', valid_games)

    # percentiles: 20, 40, 50, 60, 80, for points, fga, usage, mp
    percentiles = [0.2, 0.4, 0.5, 0.6, 0.8]
    reb_percentiles = get_percentiles(reb_totals, percentiles)
    reb_pct_percentiles = get_percentiles(reb_pct_totals, percentiles)
    orb_pct_percentiles = get_percentiles(orb_pct_totals, percentiles)
    mp_percentiles = get_percentiles(mp_totals, percentiles)

    feature_vector += reb_percentiles
    feature_vector += reb_pct_percentiles
    feature_vector += orb_pct_percentiles
    ...
    try:
        ...
        for i in range(len(recent_form_windows)):
            while (game_count < recent_form_windows[i]):
                ...
                trb_pct_list.append(float(valid_games_copy[idx]['trb_pct']))
                orb_pct_list.append(float(valid_games_copy[idx]['orb_pct']))
            ...
            orb_pct_list.sort()
            mp_list.sort()
            feature_vector+=trb_pct_list.copy()
            feature_vector+=orb_pct_list.copy()
    ...
    opp_trb_pct_totals = sum(extract_stat_from_log('trb_pct', opponent_stats_games))/(opponent_game_no-1)
    opp_trb_pct_recent = sum(extract_stat_from_log('trb_pct', opponent_stats_games[index-8:index]))/8

    opp_stats_features = [opp_off_rtg_totals, opp_off_rtg_recent, opp_def_rtg_totals, opp_def_rtg_recent, opp_trb_pct_totals, opp_trb_pct_recent]
    feature_vector += opp_stats_features

    label = float(player.gamelog[index]['trb'])
    if (label > 16.5):
        label = 17.0
    ...
    if str(season_obj.year) == "2022":
        feature_vector.append("train_rebounds")
    else:
        if (total_player_games >= 30 and player_index >= total_player_games - 10):
            feature_vector.append(player.identifier)
            feature_vector.append("test_rebounds")
        else:
            feature_vector.append("val_rebounds")
...
def create_feature_vectors(season_obj, players):
    ...
    num_found = 0
    num_not = 0
    rebounds_prop_lines = open("rebounds_prop_lines_raw.txt", "r")
    for line in rebounds_prop_lines:
        if "Date:" in line:
            ...
            file_to_write = "svm_" + data_split + "_rebounds.txt"
            ...
            # f.close()
    print(tests)
    with open('svm_test_players_rebounds.pkl', 'wb') as outp:
...
class SVM:
    ...
    def train_from_scratch(self):
        ...
        with open('./alphas_rebounds/rebounds_alphas_' + str(self.line)[0] + '.npy', 'wb') as f:
...
class fullSVM:
    def __init__(self):
        ...
        for i in range(15):
            print(i)
            with open('./alphas_rebounds/rebounds_alphas_' + str(i) + '.npy', 'rb') as f:
                alphas = np.load(f)
                c = 1.0
                svm = SVM(i, c, 0.01, "svm_train_rebounds.txt")
...
def grid_search(C_list, gamma_list, model_num):
    ...
    for c in C_list:
        for g in gamma_list:
            train_feature_vectors = open("svm_train_rebounds.txt", "r")
            val_feature_vectors = open("svm_val_rebounds.txt", "r")
            train = []
            ...
            predictions = []
            with open('./alphas_rebounds/rebounds_alphas_' + str(model_num) + '.npy', 'rb') as f:
                alphas = np.load(f)
                c = 1.0
                svm = SVM(model_num, c, 0.01, "svm_train_rebounds.txt")
                svm.train_from_alphas(alphas)
            for x in val_X:
...
# with open('svm_test_players_rebounds.pkl', 'rb') as inp:
...
#                 lines = "none"
#                 test_lines = open("svm_test_rebounds.txt", "r")
...
#         else:
#             print(value)
#     list_val = ['Stat: trb', 'none', 'none', pred_list, label_list, date_list]
#     test_player_map[player.name] = list_val

# with open('svm_test_map_rebounds.pkl', 'wb') as outp:
