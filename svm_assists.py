...
def featurize_instance(player_id, date_string, season_obj, players):
    ...
    try:
        ...
        for i in range(len(recent_form_windows)):
            while (game_count < recent_form_windows[i]):
                ...
                assists_list.append(float(valid_games_copy[idx]['ast']))
                ast_pct_list.append(float(valid_games_copy[idx]['ast_pct']))
            ...
            usage_list.sort()
            mp_list.sort()
            feature_vector+=assists_list.copy()
            feature_vector+=ast_pct_list.copy()
    ...
    label = float(player.gamelog[index]['ast'])
    if (label > 15.5):
        label = 16.0
    ...
    if str(season_obj.year) == "2022":
        feature_vector.append("train_assists")
    else:
        if (total_player_games >= 30 and player_index >= total_player_games - 10):
            feature_vector.append(player.identifier)
            feature_vector.append("test_assists")
        else:
            feature_vector.append("val_assists")
...
def create_feature_vectors(season_obj, players):
    ...
    num_found = 0
    num_not = 0
    assists_prop_lines = open("assists_prop_lines_raw.txt", "r")
    for line in assists_prop_lines:
        if "Date:" in line:
            ...
            file_to_write = "svm_" + data_split + "_assists.txt"
            ...
            # f.close()
    print(tests)
    with open('svm_test_players_assists.pkl', 'wb') as outp:
...
class SVM:
    ...
    def train_from_scratch(self):
        ...
        with open('./alphas_assists/assists_alphas_' + str(self.line)[0] + '.npy', 'wb') as f:
...
class fullSVM:
    def __init__(self):
        ...
        for i in range(15):
            print(i)
            with open('./alphas_assists/assists_alphas_' + str(i) + '.npy', 'rb') as f:
                alphas = np.load(f)
                c = 1.0
                svm = SVM(i, c, 0.01, "svm_train_assists.txt")
...
def grid_search(C_list, gamma_list, model_num):
    ...
    for c in C_list:
        for g in gamma_list:
            train_feature_vectors = open("svm_train_assists.txt", "r")
            val_feature_vectors = open("svm_val_assists.txt", "r")
            train = []
            ...
            predictions = []
            with open('./alphas_assists/assists_alphas_' + str(model_num) + '.npy', 'rb') as f:
                alphas = np.load(f)
                c = 1.0
                svm = SVM(model_num, c, 0.01, "svm_train_assists.txt")
                svm.train_from_alphas(alphas)
            for x in val_X:
...
# with open('svm_test_players_assists.pkl', 'rb') as inp:
...
#                 lines = "none"
#                 test_lines = open("svm_test_assists.txt", "r")
...
#         else:
#             print(value)
#     list_val = ['Stat: ast', 'none', 'none', pred_list, label_list, date_list]
#     test_player_map[player.name] = list_val

# with open('svm_test_map_assists.pkl', 'wb') as outp:
