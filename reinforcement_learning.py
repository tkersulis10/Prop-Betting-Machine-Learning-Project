...
def train_players(player_list, weight_alpha_divider, feature_alpha_divider):
    """
    ...
    for player_var in player_list:
        ...
        if num_games >= 20:
            ...
            weight_alpha = 1 / (num_games * num_features * weight_alpha_divider) # weight learning rate
            rolling_feature_sum = feature_values[0][0:num_features - roster_size] # ADJUST IF NEW FEATURES ADDED/DELETED
            feature_alpha = 1 / (num_games * num_features * feature_alpha_divider) # feature learning rate
            ...
            for game in range(num_games - 10):
                ...
                for feature in range(num_features):
                    if game > 0 and feature < num_features - roster_size:
                        rolling_feature_sum[feature] += (feature_alpha * feature_values[game][feature]) - (feature_alpha * rolling_feature_sum[feature])
                ...
                for weight in range(num_features):
                    weights[weight] += weight_alpha * difference * feature_values[game][weight]

            # TO DO: ADD GAMES FROM PREVIOUS SEASONS WITH HYPERPARAMETER TO WEIGH
            # DOWN PREVIOUS SEASONS/TEAMS
            player_feature_dict[player_name] = (feature_values, weights, actual_points, rolling_feature_sum, opp_team_list)
    ...
    for player in player_feature_dict.keys():
        ...
        for game in range(start_game, end_game, step):
            ...
            for feature in range(34):
                final_prediction[count] += rolling_features[feature] * weights[feature]
            # OPP TEAM INSERT HERE
            for feature in range(34, 69):
                final_prediction[count] += team_dict[opposing_team][feature - 34] * weights[feature]
...
def validation(player_list, team_list, random_tests, weight_alpha=None, feature_alpha=None, opp_team_alpha=None):
    """
    ...
    for i in range(random_tests):
        if weight_alpha == None:
            random_weight_dividers.append(random.randint(1, 500))
        if feature_alpha == None:
            random_feature_dividers.append(random.randint(1, 500))
        if opp_team_alpha == None:
            random_opp_team_dividers.append(random.randint(1, 500))
    ...
    for weight_divider in random_weight_dividers:
        for feature_divider in random_feature_dividers:
            for opp_team_divider in random_opp_team_dividers:
                ...
                player_feature_dict = train_players(player_list, weight_divider, feature_divider)

                # Get exponential rolling average of opposing team stats
                team_dict = {}
                for team in team_list:
                    team_dict[team.name] = get_opp_team_stats(team, opp_team_divider)

                # Evaluate model using divider
                results = evaluate_players(player_feature_dict, team_dict, 10, 0, 2)
                ...
                if ls_sum < best_divider_lss:
                    best_dividers = (weight_divider, feature_divider, opp_team_divider)
...
def test_players(player_feature_dict, team_dict):
    ...
    3rd and last games of the season and outputs the model's predicted points
    vs player's actual points for these games of the player's season.
    """
...
best_hyperparameters = validation(gamelog_list, team_list, 20)
with open("reinforcement_learning_output.txt", "a") as file:
    file.write("Best weight alpha found: " + str(best_hyperparameters[0]) + "\n")
    file.write("Best feature alpha found: " + str(best_hyperparameters[1]) + "\n")
    file.write("Best opposing team alpha found: " + str(best_hyperparameters[2]) + "\n")

# Train model using best hyperparameter
team_dict = {}
for team in team_list:
    team_dict[team.name] = get_opp_team_stats(team, best_hyperparameters[2])
player_dict = train_players(gamelog_list, best_hyperparameters[0], best_hyperparameters[1])
